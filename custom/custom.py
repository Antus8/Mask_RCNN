...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        json_dict = json.load(open(os.path.join(dataset_dir, "annotations.json")))
        json_list = list(json_dict.values())  # don't need the dict keys
        print("HERE-----------------------------")
        print(json_list)
        # The VIA tool saves images in the JSON even if they don't have any
        # annotations. Skip unannotated images.
        annotations = [a for a in json_list if a['regions']]
        print("annotations: {}".format(annotations))

        # Add images
        for a in annotations:
            # For each image we have a dict containing all regions
            print("a {}".format(a))
            ...
            # Extract polygon points from each region
            polygons = [r['shape_attributes'] for r in a['regions']]
            print("Extracted polygons {}".format(polygons))
            ...
            print("Extracted attributes {}".format(custom))
            ...
            for n in custom:
                try:
                    if n['ROOM']=='A' or n["ROOM"]=="B" or n["ROOM"]=="C":
                        num_ids.append(1)
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        print("In load mask num_ids {}".format(num_ids))
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            mask[rr, cc, i] = 1
        # print("GENERATED MASK IS {}".format(mask))
        # Return mask, and array of class IDs of each instance. Since we have
        # one class ID only, we return an array of 1s
        num_ids = np.array(num_ids, dtype=np.int32)
        print("In load mask num_ids number 2 {}".format(num_ids))	
